        ...
        async fullUserInformation(req, res) {
            try {
                ...
                if (age || gender || accompaniment || duration || price || difficulty || transportation || typeOfRoute) {
                    ...
                    const recomendedRoute = await axios.get(`https://api-routes-data.herokuapp.com/getRecommendation/?id=${ticket.data.user_id}`)
                    const finalRoute = await axios.get(`${GET_ROUTE_BY_ID}${recomendedRoute.data.recommended_route_id}`)
                    const updatedUser = await User.findByIdAndUpdate(req.user._id,{AIAvailable:true,recomendedRoute:recomendedRoute.data.recommended_route_id},{ new: true })
